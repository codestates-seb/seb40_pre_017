...
//        log.info("questionFindLIst= {}", questionRepository.findList(pageable,questionSearch).size());
       /* 키워드랑 태그가 달린 게시글 번호 가져오기*/
        ...
        /* 저 번호로 싹다 가져와서 페이징 */
        List<Tuple> questionTags = questionRepository.PageFindQuestionTags(pageable, questionSearch,questionIdHasKeywordAndTag);
        ...
        PageImpl<QuestionResponse> questionResponses = new PageImpl<>(questionRepository.findList(pageable,questionSearch,questionIdHasKeywordAndTag)
                .stream()
                .map(questionTuple ->
